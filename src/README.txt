...
Usage: deduper.py -t -i <inputfile> -k {"pmid"|"doi"}, use the -t option if you want to dedupe the test data instead of <inputfile>.  <inputfile> should be an absolute or relative path with the / separator, either on Windows or Unix.
...
-Duplicate Authorships
  (Included are the corresponding duplicates of Collaborators/Collaborations)
...
Future work:  I'd like to decouple the rules from the code in coreffer.py.  I'd like to define workflows independently of the code in deduper.py.
